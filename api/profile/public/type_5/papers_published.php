...
// TYPE 5 file
class Papers_published_api extends Type_5 implements api
{
    ...
    public function get()
    {
        ...
        if ($all_data) {
            ...
            send(400, 'error', 'no info about paper published found');
            die();
        }
    }
    ...
    {
        // Get the user info from DB
        $this->Papers_published->user_id = $id;
        $all_data = $this->Papers_published->read_row();
        ...
        if ($all_data) {
            ...
            send(400, 'error', 'no info about papers published found');
            die();
        }
    }
    ...
    {
        // Get data from DB
        $this->Papers_published->start = $start;
        $this->Papers_published->end = $end;
        $all_data = $this->Papers_published->read_row_date();
    ...
    public function update_by_id($DB_data, $to_update, $update_str)
    {
        if (strcmp($DB_data, $to_update) !== 0) {
            if (!$this->Papers_published->update_row($update_str)) {
    ...
    public function put()
    {
        ...
        while ($count < count($data)) {
            ...
            $at = date('Y-m-01', strtotime($data[$count]->paper_published_at));
            $this->Papers_published->from_text = $at;
        ...
        while ($count < count($data)) {
            ...
            foreach ($DB_data as $key => $element) {
                if ($element['paper_published_id'] == $data[$count]->paper_published_id) {
                    ...
                    $this->update_by_id($element['paper_published'], $data[$count]->paper_published, 'paper_published');
                    $this->update_by_id($element['paper_published_at'], $at, 'paper_published_at');
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['ID'])) {
        $Papers_published_api->get_by_id($_GET['ID']);
    } else if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1 && isset($_GET['from']) && isset($_GET['to'])) {
        $Papers_published_api->get_by_date($_GET['from'], $_GET['to']);
    } else {
