...
const papers_published = document.getElementById("papers_published");
const add_paperpub = document.getElementById("add_paperpub");
...
// -------------------------------------------- add / edit paper_published_storage  -------------------------------------------- //
// Generate unique ID
function uuid() {
...
function set_back_to_default() {
  paper_published.value = "";
  paper_published_at.value = "";
  is_international.value = "yes";
  edit_flag = false;
  edit_id = "";
  add_paperpub.textContent = "Add";
}
...
function delete_item(e) {
  ...
  papers_published.removeChild(element);
...
async function edit_item(e) {
  ...
  // set form value
  paper_published.value = edit_element.childNodes[0].innerHTML;
  paper_published_at.value = edit_element.childNodes[2].innerHTML;
  is_international.value = edit_element.childNodes[4].innerHTML;
  // faculty.value = edit_fac;
  edit_flag = true;
  edit_id = element.dataset.id;
  //
  add_paperpub.textContent = "Edit";
}
...
function clear_items(e) {
  ...
  return new Promise((resolve, reject) => {
    ...
    window.localStorage.removeItem("paper_published_storage");
    const items = document.querySelectorAll(".one-paper_published_storage");
    if (items.length > 0) {
      items.forEach(function (item) {
        papers_published.removeChild(item);
      });
    }
...
function add_previous(e) {
  ...
  if (
    ...
    element.classList.add("one-paper_published_storage");

    element.innerHTML = `
      <p class="one-paper_published"><span><b>${paper_published.value}</b> at <b>${paper_published_at.value}</b> International? <b>${is_international.value}</b></span>
      &ensp;
    ...
    // append child
    papers_published.appendChild(element);
    // display alert1
    ...
    edit_element.innerHTML = `<b>${paper_published.value}</b> at <b>${paper_published_at.value}</b> International? <b>${is_international.value}</b>`;
...
const get_local_storage = () => {
  return window.localStorage.getItem("paper_published_storage")
    ? JSON.parse(window.localStorage.getItem("paper_published_storage"))
    : [];
};
...
const add_to_local_storage = (id, deg, deg_f, deg_t) => {
  const item = {
    paper_published_id: id,
    paper_published: deg,
    paper_published_at: deg_f,
    is_international: deg_t,
  };
  let items = get_local_storage();
  items.push(item);
  window.localStorage.setItem("paper_published_storage", JSON.stringify(items));
};
...
function edit_local_storage(id, deg, deg_f, deg_t) {
  ...
  items = items.map(function (item) {
    if (item.paper_published_id == id) {
      item.paper_published = deg;
      item.paper_published_at = deg_f;
...
function setup_items() {
  papers_published.innerHTML = "";
  let items = get_local_storage();
  // console.log("local storage", items);
  if (items.length > 0) {
    items.forEach(function (item) {
      create_list_item(
        item.paper_published_id,
        item.paper_published,
        item.paper_published_at,
...
async function create_list_item(id, deg, deg_f, deg_t) {
  ...
  element.innerHTML = `
      <p class="one-paper_published"><span><b>${deg}</b> at <b>${deg_f}</b> International? <b>${deg_t}</b></span>
      &ensp;
                <button type="button" class="edit-btn btn btn-warning">
                  <i class="fas fa-edit"></i>
                </button>
...
function submit_form(e) {
  ...
  const paper_published_storage = get_local_storage();

  // // check if empty
  // if (paper_published_storage.length <= 0) {
  //   display_alert("please add some present responsibilities", "danger");
  //   return;
  // }

  paper_published_storage.forEach((item) => {
    item.paper_published_id = isNaN(Number(item.paper_published_id))
      ? 0
      : item.paper_published_id;
  ...
  xhr.open(
    "POST",
    `../../api/profile/public/type_5/papers_published.php?ID=${user.user_id}`,
    true
  );
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == XMLHttpRequest.DONE) {
      ...
      if (got.error) {
        ...
        window.alert("Papers published updated successfully");
        got.forEach((item, index, array) => {
          item.paper_published_at = item.paper_published_at.substr(0, 7);

          if (index + 1 == array.length) {
            // assign the data
            window.localStorage.setItem(
              "paper_published_storage",
              JSON.stringify(got)
            );
            setup_items();
          }
...
// add an paper_published_storage
add_paperpub.addEventListener("click", add_previous);
// when form submitted
paperpub_form.addEventListener("submit", submit_form);
// initialize
